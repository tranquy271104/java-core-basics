...
- Kiểm tra kí tự in thường: if (c >= 'a' && c <= 'z') hoặc if (c >= 97 && c <= 122)
- Kiểm tra kí tự in hoa:    if (c >= 'A' && c <= 'Z') hoặc if (c >= 65 && c <= 90)
- Kiểm tra kí tự là chữ số: if (c >= '0' && c <= '9') hoặc if (c >= 48 && c <= 57)
- Đổi thường thành IN HOA (Trừ 32 đơn vị): c = (char) (kyTuThuong - 32);
- Đổi hoa thành in thường (Cộng 32 đơn vị): c = (char) (kyTuHoa + 32);
